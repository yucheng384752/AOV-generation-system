...
import { ReactFlow, Background, Controls, MiniMap, Handle, Position, ConnectionMode, MarkerType, BaseEdge, EdgeLabelRenderer, useUpdateNodeInternals, applyNodeChanges, type EdgeProps, type NodeProps, type Node, type Connection, type ReactFlowInstance } from '@xyflow/react';
...
export default function Canvas(props: Props) {
  ...
  const mapped = useMemo<FlowNode[]>(() => {
    const make = (n: AovNode, id: string): FlowNode => ({ id: n.id, type: 'aov', position: doc.layout[id].positions[n.id], data: { item: n, workflow: doc.documentType === 'workflow', color: inheritedColor(doc, id, n), mode: props.mode }, selected: n.id === selected });
