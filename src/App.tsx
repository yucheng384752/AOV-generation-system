...
function DocumentEditor({ editor, companion, active, onSwitch, onImport }: { editor: Editor; companion: Editor; active: boolean; onSwitch: (type: AovDocument['documentType']) => void; onImport: (doc: AovDocument) => void }) {
  ...
  return <div className="app-shell" id={`${doc.documentType}-editor`} onClickCapture={guardUnapplied} onDoubleClickCapture={guardUnapplied}>
    ...
    {(activeNode || activeEdge || panel) && <aside className="inspector" key={selected ?? panel}><button className="close-panel" aria-label="關閉設定面板" onClick={() => { select(null); setPanel(null); }}>×</button>
      ...
      {panel === 'project' && <><span className="eyebrow">PROJECT DEFINITION</span><h2>專案設定</h2><TextField label="專案名稱" value={doc.project.name} onChange={name => update(d => { d.project.name = name; })} /><TextField label="系統目的與範圍" value={doc.project.description} multiline onChange={description => update(d => { d.project.description = description; })} /><TextField label="目前畫布名稱" value={graph.name} onChange={name => update(d => { d.graphs.find(g => g.id === graph.id)!.name = name; })} /><p className="note">本工具只定義架構，不實作或執行所描述的功能。</p><small>草稿儲存在此瀏覽器。本地 .aov.json 才是可攜的長期保存檔。</small>{graph.id !== doc.rootGraphId && <button onClick={() => update(d => syncBoundaries(d))}>重建／同步邊界入口出口</button>}</>}
