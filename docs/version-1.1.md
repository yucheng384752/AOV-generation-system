...
正向線只能從畫面左方 node 的右側輸出接點連到右方 node 的左側輸入接點。返回線只能從畫面右方 node 的單一下方接點連回左方 node 的單一下方接點，箭頭指向左方目標；反向拖曳會被阻擋並顯示原因。移動既有節點不會自動改寫連線語意或刪除連線。

## 1.1.3 接點與必填標示

正向模式採左進右出；返回模式採右進左出。兩種接點共用節點左右邊界，依工具列模式只顯示目前可使用的一組；返回虛線在來源與目標附近向下轉折後連接。節點卡片不顯示「前一步／下一步」或其他 port 名稱，契約名稱仍保存在設定面板與 JSON 中。完整性檢查所要求的欄位在標題後顯示紅色 `*`，並套用原生 required 屬性。

## 1.1.4 下方返回線

1.1.4 取代 1.1.3 的左右返回接點。每個節點在返回模式只顯示一個下方接點，同一點可作為起點或終點；由右方來源向下延伸，以帶圓角的水平線向左，再向上接入左方目標，箭頭位於目標下方。正向接點、必填星號及隱藏 port 文字的行為不變。

## 1.1.5 返回線中間節點

返回線可由多條 return edge 串接中間處理節點。起點與終點仍使用各節點唯一的下方返回接點；中間節點由右側返回入口接收，再由左側返回輸出繼續向左連接。每段 edge 各自保存 `sourceAnchor` 與 `targetAnchor`，可用值分別為 `bottom | left` 與 `bottom | right`，以確保匯入後能還原接點位置。舊 1.1 檔缺少錨點時視為 `bottom → bottom`。
