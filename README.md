...
正向線由左方 node 的右側輸出連到右方 node 的左側輸入；返回線由右方 node 的單一下方接點連回左方 node 的單一下方接點，以虛線顯示。中間節點可以同時連接前後節點。先使用工具列切換正向／返回模式，再拖曳對應接點。返回接點沿用具名輸入／輸出契約，JSON 檔案格式仍為 1.1。

完整性檢查只要求辨識與結構安全所需內容：專案／節點／連接埠名稱、外部引用名稱與位置、返回原因，以及重複 Loop 的上限、停止與超限處理。用途、步驟、條件、映射與各類說明可按需要補充。
